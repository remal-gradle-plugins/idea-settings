...
- Modifiable sections: lines between `$$$sync-with-template-modifiable: <name> $$$` and `$$$sync-with-template-modifiable-end$$$` markers keep their local content across syncs. Keep the markers. See [modifiableSections.ts](https://github.com/remal-github-actions/sync-with-template/blob/main/src/internal/modifiableSections.ts) for the full syntax.
